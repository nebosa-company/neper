...
A registry `code` identifies a diagnostic meaning across compiler versions. It is
not a diagnostic occurrence. The planned T2 tooling-v2 contract therefore also
gives each occurrence a result-local `id` and a versioned, position-independent
`fingerprint` for cross-invocation correlation. Neither changes this registry or the
closed v1 stream; neither may be used in place of a source snapshot, edit
precondition or evidence-backed verification receipt.
...
| `E-SAFETY-0023` | an `extern` result or mutable pointee, or a `@cc` callback's parameter or pointee, of a type that admits only its members (D920) | spec §§5, 11 |
| `E-SAFETY-0024` | an untagged `union` with more than one field, one of a type that admits only its members (D921) | spec §11 |
...
| `E-LINK-0002` | `e.mem.Arena` is not laid out as the embedded runtime reads it: `base` at 0, `cap` at 8, `off` at 16, twenty-four bytes (D552) | spec §8 |
...
| `E-TOOL-0003` | an edit plan that cannot be applied as it stands: a file changed since the plan was made, an edit with no precondition or outside its file, or a plan that did not succeed; nothing is written (D481) | tooling §7 |
