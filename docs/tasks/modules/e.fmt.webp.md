# e.fmt.webp — 8 of 8 declarations missing

| field | value |
|---|---|
| file to create | `lib/e/fmt/webp.e` |
...
Implement **exactly** the public fence below in `lib/e/fmt/webp.e`; nothing more, nothing less, same spelling, same order. A module is delivered only when `python scripts/check_module_surfaces.py` finds every declaration, its `modules.json` row moves to `surface:"source"` in the same commit, and a `link/<module>` fixture proves the behaviour on both hosts (README §Fixture template).
...
- `build/windows/tests/selfhost/neper-self.exe parse-file lib/e/fmt/webp.e` prints `parse file ok`.
- A fixture `tests/selfhost/fixtures/link/fmt_webp/src/main.e` that prints one fixed line on success, registered in both runners.
