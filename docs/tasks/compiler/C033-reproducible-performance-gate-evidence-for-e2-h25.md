...
- [ ] the H25 report -- the workflows m2-baseline.md leaves pending (local edit/revert, rename/move/delete, context/repair, persistent session, CPU program runtime), each frozen with its workload and budget and measured on both hosts, GPU marked pending with its M3 owners (H25 requires one before closure), Linux measurements after the baseline
- [ ] the H25 report
