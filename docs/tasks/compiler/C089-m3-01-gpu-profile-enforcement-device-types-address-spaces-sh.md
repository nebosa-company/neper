...
| score | 0.15 of 1 |
| queue position | 28 of 55 (only position 1 is eligible for the next session; see README) |
...
> The `@gpu(X, Y, Z)` attribute is read at signature collection: one to three positive integer literals with a product of at most 1024, on a non-extern function, refused otherwise under one diagnostic; a kernel called anywhere but `gpu.launch` is refused at the call (D778)

## Remaining work

The queue's own gap clause, split into checklist lines. Each line is one session's target.

- [ ] the call-chain profile check inside a kernel, device types and `[]shared T`, `shared var`, `caps(...)` and `ftz`
...
- `caps`: `lib/e/text/regex.e`×7, `src/main.e`×5‡, `benchmarks/scale/elf_stub.py`×2, `benchmarks/baseline/measure.py`×1, `lib/e/algo/bignum.e`×1, `lib/e/fmt/html.e`×1†, `lib/e/fs.e`×1, `lib/e/os.linux.e`×1†
