...
compiler = {
 "Front end and language": [
  ...
  ("Atomic[T] and memory orderings", 0, "the prerequisite landed: an instantiation used as a field type checks (check/generic_instance_field), which is the shape e.sync needs. Atomic itself has not started -- the thirteen e.atomic intrinsics inferred from their pointer, per-operation ordering rules and lock-prefixed codegen. Blocks e.sync and e.channel"),
