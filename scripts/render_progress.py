...
compiler = {
 "Front end and language": [
  ...
  ("General comptime interpreter", 0.25, "integer const folding only. It is what a self-recursive generic over a type now waits on: `encode[T]` recursing into `encode[meta.element_type[T]()]` terminates only if the `meta.kind[T]()` branch that guards it is folded away, and a branch on a non-integer constant is not (D136)"),
 ],
 "Back end": [
