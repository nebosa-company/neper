...
compiler = {
 "Front end and language": [
  ...
  ("Checked optimized builds and unsafe boundaries (M2.5 H03)", 0.5, "A release build keeps `bounds`, `null`, `tag` and `align` (D355, `m25-h03-checked-release.md`); `@nocheck` is the explicit unchecked operation in every mode and `--unchecked` the whole-image one; the manifest lists every `@unsafe` function and `@nocheck` block and the check policy; a fixture traps in release on all three memory rows. Not yet: check elimination with proofs and its codegen fixtures, `invalid` in release, definite initialization of fields, the load/store audit; cold wall +19-34% and image +36% against budgets of +10%/+5%"),
