...
compiler = {
 "Front end and language": [
  ...
  ("Bounded, versioned harness transport (M2.5 H18)", 0.45, "One versioned JSONL stream with a header, a closed record set and `result.data` as the extension map, validated against `docs/schemas/neper-v1.schema.json` by the suites (D227-D300); two-site diagnostics carry `related` (D364); `run --json` is bounded -- `--capture N` bytes per stream in the record, whole counts and `captured_complete` in the result, the files beside the executable holding the rest (D370); a build past `--deadline` ends with one diagnostic and exit 3 (D399); a context page reports its serialized `bytes` and takes a byte budget (D400), and its cursor is bound to the program's `snapshot` on the subject record (D407). Not yet: streamed capture chunks, typed diagnostic facts, sequence IDs and progress records, a versioned `fix` with expected-source preconditions"),
