...
tooling = [
 ...
 ("build", 0.65, "`emit-executable PATH ROOT ARCH OS OUTPUT [--release] [--arena SIZE] --json` (D230): the section 1 header with command `build`, every front-end, lowering, code-selection and error-table diagnostic as a record, and the result naming the executable as it was given; tests/conformance/tools pins a program that builds and one that is rejected. Every successful build writes `.neper/<mode>/build-manifest.json` under the project root when that directory exists, its one artifact the executable as named with the SHA-256 of the bytes written (D254); both suites make the directory, validate the manifest against the schema and compare the hash to the file on disk. `--project DIR` names the project root outright instead of discovering it from the operand (D263), so a file outside the tree -- a generated test runner -- is built as part of it. Not yet: making `.neper/<mode>/` (the fixed os surface has no mkdir), `--target` and `-o` spellings, a project root as operand, warnings"),
