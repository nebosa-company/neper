...
compiler = {
 ...
 "Back end": [
  ...
  ("Work-stealing pool, parallel parse and codegen", 0.6, "the front end runs in waves of eight workers (D321) and the modules are lowered, selected and written as artifacts on eight workers with checkers that share the declarations (D325); a two-million-line program builds cold in 10.4 s where it took 29, and the bootstrap runs a thread inline. The body sweep is single-threaded, and the work is assigned statically, not stolen"),
