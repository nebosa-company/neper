...
compiler = {
 ...
 "Back end": [
  ...
  ("Work-stealing pool, parallel parse and codegen", 0.75, "the front end runs in waves of eight workers (D321); the modules are lowered, selected and written as artifacts on eight workers with checkers that share the declarations (D325); and the same workers check the bodies and build both inlining oracles first, each with a checker forked from the program's and importing the types of a body copied from another worker's oracle (D326). A two-million-line program builds cold in 7.6 s debug and 7.4 s release where it took 29 and 26, its body sweep 0.6 s where it took 1.9 (1.3 s where it took 5.7 in release), the settle nothing on a cold build and the lowering not re-checking what the sweep checked (D327), and the bootstrap runs a thread inline. The names are validated on the workers too (D328), and the artifact writer copies bytes with one runtime call (D329, 500k lines cold 1.4 s); what remains sequential is collecting the declarations; the work is assigned statically, not stolen"),
