...
tooling = [
 ("JSONL v1 stream envelope and version header", 0.7, "The header, diagnostic, token, syntax and result records of section 1 (D227, D228), each a line, the result last with the exit status; emitted by `tokens`, `parse` and `check-file`. Not yet: `build`, `run`, `test`, `fmt`, `index`, `dis`, `info`"),
 ...
 ("Conformance corpus accept/reject/format/tokens/parse/tools", 0.5, "tests/conformance/tokens, parse, accept and reject exist with byte-exact expected streams the suites compare on both platforms (D227, D228); format and tools do not"),
