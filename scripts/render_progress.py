...
compiler = {
 ...
 "Back end": [
  ...
  ("Cross-module inlining, 40 NIR cap", 0.9, "The inlining oracle (D207): every short non-generic function of every module is lowered ahead of the program into a builder of its own, and a call to one that came out at forty NIR instructions or under, with at most one register result, is replaced by a copy of its body -- parameters become the arguments, returns become branches to the continuation, references are re-interned -- in module order on both link paths, so an executable linked from artifacts is still byte-identical. The artifact records a body edge to every inlined callee of another module, and `--incremental` acts on it: link/incremental pins a body edit behind a signature edge keeping the dependent and one behind a body edge rebuilding it. Not yet: the cap is applied within a module too, where the section has none, and a callee inlined into a callee is not inlined again"),
  ("Incremental rebuild on the edge rule", 0.6, "`emit-em-all --incremental` (D205) compiles the program and then lets section 12's edge rule decide per module: the artifact on disk stays when its source hash is unchanged and every recorded edge still matches the declaration it names in the target's fresh artifact, and is replaced otherwise; `kept`/`rebuilt` is printed per module. link/incremental pins unchanged sources kept, a body edit behind a signature edge rebuilding only its module with the linked result byte-equal to a clean build, and a signature edit rebuilding the dependent, on both platforms. What is left is the saving: every module is still compiled and only the writes are skipped, because checking one module against its dependencies' Interfaces alone, without their sources, is not a path the checker has"),
  ...
  ("Determinism harness", 0.5, "compiler fixed point on both platforms, artifact-linked equals source-linked for every artifact fixture, and incremental equals clean for link/incremental (D205); no -jN case, there being no threads"),
