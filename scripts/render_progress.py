...
compiler = {
 "Front end and language": [
  ...
  ("Compilation policy and generated-code performance (M2.5 H20)", 0.5, "Three policies, each a build identity and named in the manifest: debug, release with the memory checks retained, release `--unchecked` (D355, D369); the forty-instruction cap re-evaluated on the 500k-line workload and retained, `--inline-cap N` and `--explain` giving every inlining decision its reason (D346), and under `--json` as `inline` records of the build stream, gathered per worker and written in worker order, pinned per host (D408); the bounds proof and the `--stats` rows for elided checks and by-value copies (D356); measured budgets and the gate (D338, D366). Not yet: copies/allocations/register-pressure measurements, lazy `.em` section access, serialized alias facts, the cap against the GP workloads, any vectorizer (reported unavailable)"),
  ...
  ("Protocols, generics and compile-time context (M2.5 H06)", 0.45, "`explain-file --json` (D359) emits every protocol dispatch -- declared, supplied or none -- and every generic instantiation with its arguments and requesting site, from the checker's own decisions; pinned by the corpus; comptime evaluation is bounded at ten million steps and a call depth of sixty-four, each exhaustion an E-COMPTIME-9999 naming the constant and the budget (D218), every compiler table full is a named limit, never a successful check (D344), and `--instances N` is a specialization budget of its own -- a build past it is refused with the count (D426); a dispatch that found nothing says why the supplied rule refused and names a candidate declared outside the receiver's module, in the explain stream -- which a failing program now answers up to the failure -- and in the diagnostic (D430). Not yet: dependent requirements, explicit strategy selection, layout and phase in the output, per-instance cost"),
