...
compiler = {
 "Front end and language": [
  ...
  ("Regions, borrows and container views (M2.5 H02)", 0.45, "The lexical subset (D354, `m25-h02-regions.md`): a mark is followed, values allocated after it dangle at the reset, views of a container dangle at its mutation, both refused at the use (E-SAFETY-0013/0014) with the join and loop rules of H01; no false positive across the compiler, library and fixtures; a pointer local bound from `&x` is recorded as an alias of `x`, and the lending rule follows it (D393). Not yet: the region and view rules through that alias, borrow summaries across modules, escapes through structs, callbacks and threads, non-lexical liveness, build-then-freeze containers, generation-tagged handles"),
