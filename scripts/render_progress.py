...
compiler = {
 ...
 "Back end": [
  ...
  ("Work-stealing pool, parallel parse and codegen", 0.3, "the front end runs in waves of eight workers (D321): every module a wave discovered is lexed and parsed by a worker into an arena of its own, nothing written is shared and no lock exists; a two-million-line program loads and parses in 1.0 s where it took 4.6, and the bootstrap runs a thread inline. Checking, lowering and codegen are single-threaded, and there is no work-stealing pool"),
