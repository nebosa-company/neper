...
tooling = [
 ("JSONL v1 stream envelope and version header", 0.6, "The header, diagnostic, token, syntax and result records of section 1 (D227), each a line, the result last with the exit status; emitted by `tokens` and `parse`. Not yet: the other commands, `info`, and stderr discipline on a failed JSON start"),
 ...
 ("Conformance corpus accept/reject/format/tokens/parse/tools", 0.3, "tests/conformance/tokens and parse exist with byte-exact expected streams the suites compare on both platforms (D227); accept, reject, format and tools do not"),
