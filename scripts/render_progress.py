...
compiler = {
 ...
 "Back end": [
  ...
  ("Work-stealing pool, parallel parse and codegen", 0.75, "the front end runs in waves of eight workers (D321); the modules are lowered, selected and written as artifacts on eight workers with checkers that share the declarations (D325); and the same workers check the bodies and build both inlining oracles first, each with a checker forked from the program's and importing the types of a body copied from another worker's oracle (D326). A two-million-line program builds cold in 9.7 s where it took 29, its body sweep 0.7 s where it took 1.9 (1.5 s where it took 5.7 in release), and the bootstrap runs a thread inline. What remains sequential is resolve, the declarations and the settle; the work is assigned statically, not stolen"),
