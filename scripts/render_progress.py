...
compiler = {
 "Front end and language": [
  ...
  ("e.meta reflection", 0.9, "link/meta_scalar answers kind, array_len and type_name; link/meta_reflect walks fields and members through comptime Field and Member values, an unrolled for and get/set at the named offset; link/meta_types answers element_type and backing_type, whose result is a type usable as a comptime argument, nested in the other question, or instantiating a generic. Two gaps: the type grammar admits no call, so the answer cannot be a local's annotation, and `Field`/`Member` are not declared names -- a user declaration cannot take `[FIELD: Field]` until the generic-argument path accepts a comptime binding"),
