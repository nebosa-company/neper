...
compiler = {
 "Front end and language": [
  ...
  ("Resource ownership and cleanup (M2.5 H01)", 0.7, "The seeded handles are affine and obligated, `own` parameters transfer, every exit audits, `defer` reserves, an untested acquisition is unchecked, arms join and loops keep outer resources (D345); declared `resource(cleanup)` types, containment followed field by field, views, and a declared resource's fields its module's (D348): nine E-SAFETY codes pinned by conformance fixtures, both suites on every fixture and library, the compiler's own leaks found and fixed. Not yet: copies through generics, borrow-then-move, closing a borrowed handle, arrays of resources, `os.dup`, `mem.Arena`, the seeded handles' opacity"),
