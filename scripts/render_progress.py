...
compiler = {
 "Front end and language": [
  ...
  ("Protocols, generics and compile-time context (M2.5 H06)", 0.35, "`explain-file --json` (D359) emits every protocol dispatch -- declared, supplied or none -- and every generic instantiation with its arguments and requesting site, from the checker's own decisions; pinned by the corpus; comptime evaluation is bounded at ten million steps and a call depth of sixty-four, each exhaustion an E-COMPTIME-9999 naming the constant and the budget (D218), and every compiler table full is a named limit, never a successful check (D344). Not yet: failed candidates with reasons, dependent requirements, explicit strategy selection, a specialization-count budget of its own, layout and phase in the output, per-instance cost"),
