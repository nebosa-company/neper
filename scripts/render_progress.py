...
The four readiness numbers are computed, never typed: the compiler and tooling
scores come from the tables below, the module and UI/host scores come from their
machine-readable inventories. Post-M2 track status is rendered separately from
docs/hardening-tracks.json and is never inferred from precursor capability scores.
"""
...
tooling = [
 ...
 ("Module-plan validation in CI", 1, "check_module_plan.py 150 modules; check_module_surfaces.py 28 compiler-resolved sources"),
 ...
 ("`--stats` as a stream record", 1, "With `--json`, `--stats` is one flat `stats` record on stdout before the result (D476): one snake_case key per table row with the unit as its suffix, scalars only, phases as `phase_<name>_ms`, `--stats-full`'s pools as `pool_<name>_capacity` and `_used`, rendered by the same row helpers as the stderr table; `build --json --stats` printed nothing before. Schema entry; both suites validate the record"),
 ("Image digest reused on a warm build", 1, "D332: the executable's SHA-256 was the one hash a warm build still ran, sixty milliseconds of the compiler's 189. Before the write, `source.matches` compares the packed image with the file at the output path -- the size from a seek to its end first, the way `load` sizes its buffer, and only then the bytes a chunk at a time, stopping at the first that differs -- and, equal, `save_executable` is not called, so the file's mtime stays and nothing watching the binary wakes. `tool.manifest_previous_digest` then takes the artifact's `sha256` from the existing `.neper/<mode>/build-manifest.json` when its one artifact is this path, by the key scan the manifest's other readers use (D255); a manifest missing, unreadable or naming another artifact hashes as before, as does any image that differs. The answer rides `report.build.image_unchanged` rather than a local: `dispatch` is at the bootstrap's 256-local ceiling. Pinned beside the reproducible-build check (D261) in both suites: a third build of the same source into the same output leaves the mtime untouched and its manifest carries the image's digest"),
 ("Compiler hashes from the library", 0.4, "xxHash64 and FNV-1a land (D747): `src/artifact_hash.e` calls `e.algo.hash` for both, and the tuned inline-load one-shot D388 wrote for the artifact path moved into the library rather than the library's byte-buffered one surviving -- D333's rule that the faster moves. `round` (the `program_snapshot` fold, D407) and `fnv1a32_step` (the module-dot-name fold, D6) stay in the compiler with `rotate_left` and `xor` under them; `read_u32`, `read_u64`, `merge_round` and three primes went with the body. Pinned by a seventy-seven byte vector in `artifact_hash.self_test` -- two whole blocks, then an eight, a four and a one -- and by the suite's artifacts, manifest goldens and stage-2/stage-3 fixed point. `e.algo.hash` is `surface:\"source\"`, so the move added no declaration. Not yet: CRC-32C, which needs the table-driven `crc32c` D333 plans before the compiler's slice-by-8 with its zeroed window and its SSE4.2 path (D320, D332) can go; and SHA-256, which is blocked -- the C bootstrap builds stage one on every suite run and refuses `lib/e/crypto/hash.e` with nineteen E-TYPE-0002s where a `u64` shifts by a `u32`, so that half waits on the bootstrap's deletion (D208), which is the finding D333 asked for"),
 ("Reproducible-build check", 0.85, "Both suites check the compiler's fixed point -- stage 2 and stage 3 byte for byte -- and, since D261, that a program built twice is the same executable and that the second build's manifest carries the first's artifact SHA-256, so two builds can be compared by their manifests without the executables (D254 wrote the hash); `neper compare-manifests A B [--json]` (D482) is that comparison as a command -- identity, inputs, dependencies and artifacts by hash, not path -- and both suites hold two builds' manifests to agree and a debug one to differ from a release one by mode and artifact. Not yet: cross-host comparison (a Windows and a Linux build differ by design), and the dependency and library hashes the manifest does not yet carry"),
 ("Generated-code benchmark corpus", 0.5, "benchmarks/llm_edit"),
 ("Self-host regression suite, both platforms", 1, "305 fixtures, run.ps1 + run.sh"),
