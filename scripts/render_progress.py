...
compiler = {
 "Front end and language": [
  ...
  ("Scoped concurrency and shared-state contracts (M2.5 H04)", 0.6, "The design (`m25-h04-concurrency.md`, D365): the join obligation is H01's; a thread over this frame's storage cannot be detached, handed on, returned or stored past it (D357, E-SAFETY-0015); what a thread was given is lent to it until the join, read or written by nobody but through an address (D365, E-SAFETY-0016); a lock held as a value is a `sync.Guard` resource, released on every exit by H01's rule (D379); a pointer local bound from `&x` is followed, so a read or store through it while `x` is lent is refused (D393); the compiler's crews and every fixture pass. Not yet: the protected data as a view of the guard, RwLock guards, aliases through slices and globals, partial spawn failure over arrays, a perturbation fixture"),
