...
compiler = {
 "Front end and language": [
  ...
  ("Regions, borrows and container views (M2.5 H02)", 0.6, "The lexical subset (D354, `m25-h02-regions.md`): a mark is followed, values allocated after it dangle at the reset, views of a container dangle at its mutation, both refused at the use (E-SAFETY-0013/0014) with the join and loop rules of H01; no false positive across the compiler, library and fixtures; a pointer local bound from `&x` is recorded as an alias of `x`, and the lending rule (D393), the region rule and the view rule (D394) follow it, `.len` excepted, a slice bound from a place of `x` is the same alias (D395), and a struct local given `&x` in a field -- by its literal or by a store -- aliases `x` through that field alone (D413). Not yet: aliases rebound under the alias, a struct with addresses in two fields (the first is followed), borrow summaries across modules, escapes through structs, callbacks and threads, non-lexical liveness, build-then-freeze containers, generation-tagged handles"),
