...
compiler = {
 "Front end and language": [
  ...
  ("Diagnosis, recovery and transactional repair (M2.5 H09)", 0.4, "Every two-site E-SAFETY diagnostic carries its other site -- acquisition, move, deferred call, borrow, pointer, reset, mutation, thread start -- as a `related` span with a message saying which (D364), pinned by the reject corpus; stable codes throughout; the parser's nesting bound and error nodes are D341's; a forgotten cleanup carries its `defer <closer>(x)` as the first `fixes` entry (D381), and a rename is a plan with preconditions (D376). Not yet: fixes for the other codes, cascades grouped under a primary cause, expected/actual as fields, instantiation chains, lossless recovery beyond D341"),
