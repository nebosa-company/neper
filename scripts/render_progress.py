...
compiler = {
 "Front end and language": [
  ...
  ("Compiled LLM cards (M2.5 H28)", 0.7, "`docs/llm-neper-card.md` is the render of `scripts/render_card.py`: the grammar's own productions, its keyword set, the diagnostic registry's families and the stream versions, around the prose of `llm-neper-card.src.md`, stamped with the grammar revision and a content hash, and both suites refuse a card that is not the render (D374). per-symbol API records are `context-file --module` (D397) and the card's examples are checked by both suites (D404). Not yet: planned/present/verified marking, one card per language version"),
