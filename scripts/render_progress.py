...
compiler = {
 "Front end and language": [
  ...
  ("Checked optimized builds and unsafe boundaries (M2.5 H03)", 0.6, "A release build keeps `bounds`, `null`, `tag` and `align` (D355, `m25-h03-checked-release.md`); `@nocheck` is the explicit unchecked operation in every mode and `--unchecked` the whole-image one; the manifest lists every `@unsafe` function and `@nocheck` block and the check policy; a fixture traps in release on all three memory rows; the first bounds proof (`while i < x.len`, D356) with its eliminated and retained fixtures and a `--stats` count, which also found and closed the inlined bodies' lost checks. Not yet: proofs through a second local or a field base, `invalid` in release, definite initialization of fields, the load/store audit; cold wall +16-29% and image +40% against budgets of +10%/+5%"),
  ("Reproducible performance gate (M2.5 H25)", 0.6, "Registered workloads, budgets and rendered reports (D338, `m2-baseline.md`); `benchmarks/baseline/gate.py` judges a measurement cell by cell against the budgets and exits 1 on a breach (D366) -- twelve named on the D351 measurement, the D340 and D355 breaches the decision rows carry. Not yet: the gate in the suites, Linux measurements after the baseline, the H25 report"),
  ("Unsafe boundaries enumerable (M2.5 H27)", 0.7, "The build manifest's `unsafe` array lists every `@unsafe` function and `@nocheck` block with module, function and line, read off the modules' bytes so warm builds list them too, and `options.checks` says the policy (D355); `build-manifest-file` gives it without a build. Not yet: provenance per boundary beyond the kind, the index marking them, `--unchecked` images listed as one boundary in the context records"),
  ("Diagnosis, recovery and transactional repair (M2.5 H09)", 0.3, "Every two-site E-SAFETY diagnostic carries its other site -- acquisition, move, deferred call, borrow, pointer, reset, mutation, thread start -- as a `related` span with a message saying which (D364), pinned by the reject corpus; stable codes throughout; the parser's nesting bound and error nodes are D341's. Not yet: cascades grouped under a primary cause, expected/actual as fields, instantiation chains, lossless recovery beyond D341, structured fixes and transactions"),
  ...
  ("Semantic search and safe change planning (M2.5 H17)", 0.3, "`uses-file --json --symbol module.name` (D362): every use the checker resolved -- calls of the function and its instances, dispatches that chose it, instantiations, its name as a value -- with the enclosing function and span, the roots that keep it alive, the count of calls through values that no name can trace, and a completeness flag; the contract says what a safe-delete claim and a rename plan may conclude. Not yet: field and address relations, ownership and borrow uses, edit plans with obligations, moves and error renames, the shadowing and alias fixtures"),
  ...
  ("Scoped concurrency and shared-state contracts (M2.5 H04)", 0.45, "The design (`m25-h04-concurrency.md`, D365): the join obligation is H01's; a thread over this frame's storage cannot be detached, handed on, returned or stored past it (D357, E-SAFETY-0015); what a thread was given is lent to it until the join, read or written by nobody but through an address (D365, E-SAFETY-0016); the compiler's crews and every fixture pass. Not yet: locks and guards as regions over the mutex, aliases through slices, pointer locals and globals, partial spawn failure over arrays, a perturbation fixture"),
