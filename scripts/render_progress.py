...
compiler = {
 "Front end and language": [
  ...
  ("Learnability and executable API documentation (M2.5 H11)", 0.3, "The index carries every symbol's signature, spans, attributes and documentation (D232), `context-file` its resource and unsafe facts (D361) and, from the signature, the caller's contract -- allocation from an arena parameter, borrow or mutation per pointer parameter as the view rule reads it, fallible or partial results, a thread started in the body (D396) -- and the language card is generated and stamped (D374). Not yet: per-symbol records as a catalogue rather than one query at a time; planned/present/verified marking; examples that compile under the suite; near-miss negative examples; the stdlib-hardening migrations of SL01-SL11 as a gate"),
