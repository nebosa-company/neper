...
compiler = {
 "Front end and language": [
  ...
  ("Vec[T,N] / Mask[T,N] and SIMD lowering", 0.4, "`Vec[T, N]` and `Mask[T, N]` are builtins seeded into e.simd with section 4's closed table, width-aligned layout and `meta.kind/element_type/array_len` answers; every `simd.*` intrinsic but `shuffle` is library source lowered lane by lane over scalar code (D148). Not yet: the lane-wise operators, the `Vec{}` literal and `v[i]`, the mask register-only rule, `shuffle`'s comptime array, and a vector register class with SSE/AVX selection"),
