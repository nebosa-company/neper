...
# Each item scores 1 (done), 0.5/0.25 (partial), 0 (not started).
compiler = {
...
<footer>
  Generated __DATE__ by <code>scripts/render_progress.py</code> from
  <code>docs/roadmap.md</code>, <code>docs/module-apis.md</code>,
  <code>docs/modules.json</code> and the compiler sources at <code>__REV__</code>.
</footer>
...
for key, val in [
    ...
    ('__DATE__', datetime.date.today().isoformat()), ('__REV__', rev),
]:
