...
tooling = [
 ...
 ("build", 0.75, "`emit-executable PATH ROOT ARCH OS OUTPUT [--release] [--arena SIZE] --json` (D230): the section 1 header with command `build`, every front-end, lowering, code-selection and error-table diagnostic as a record, and the result naming the executable as it was given; tests/conformance/tools pins a program that builds and one that is rejected. Every successful build writes `.neper/<mode>/build-manifest.json` under the project root when that directory exists, its one artifact the executable as named with the SHA-256 of the bytes written (D254); both suites make the directory, validate the manifest against the schema and compare the hash to the file on disk. `--project DIR` names the project root outright instead of discovering it from the operand (D263), so a file outside the tree -- a generated test runner -- is built as part of it. Spec section 2's spelling works (D276): `neper build FILE [-o OUT] [--target ARCH-OS] [--release] [--json]`, with the toolchain root the binary's own directory, the target the host unless said, and the output the operand's stem (`.exe` on Windows); `run`, `check`, `fmt`, `index`, `dis` and `build-manifest` have the same front door; both suites compare the short `build` stream to the positional form's golden. Not yet: making `.neper/<mode>/` (the fixed os surface has no mkdir), a project root as operand, a short `test` (its WORKDIR has no short answer), warnings"),
