...
compiler = {
 "Front end and language": [
  ...
  ("Error detail and partial failure (M2.5 H07)", 0.5, "A failing cleanup records its detail only over a read slot, so the acquisition's detail survives the close on an error path (D360, both hosts, pinned by a fixture that exits 12 against the old library); the fixed surface's outputs on failure, consumption and retry are specified; a dropped `err` is a `discard` record in `explain-file`; `stat_detail`, `dir_open_detail` and `open_detail` write the failure into the caller's `ErrorDetail` at the call, with no order to keep, on both hosts, pinned by the error-detail fixture (D417). Not yet: the `_detail` form for the rest of the surface, wrappers beyond `e.os`, concurrent errors, per-step allocation failure"),
