...
compiler = {
 "Front end and language": [
  ...
  ("Hostile inputs, artifact integrity and aggregate limits (M2.5 H24)", 0.55, "Every artifact read is checksum-validated once at load and layout-validated by each reader (D213, D320); `benchmarks/fuzz/fuzz.py` mutates every fixture source and artifact and found nothing in 33k runs, and expression nesting is bounded at 128 levels (D341); artifacts are published by one atomic replace and a damaged cache is rebuilt to the clean image, both suites (D343); an escaped resource limit is E-TYPE-9999 (D344); a corrupt artifact named on the command line is E-LINK-0001 and a damaged cache entry says `invalid-artifact` in the manifest (D368). `--fault-write N` makes one artifact write die between staging and replace, and both suites read the next build: the faulted module alone rebuilt, the image the clean build's (D435). `--instances N` is a whole-build budget over every worker's instantiations (D426). Not yet: content integrity for imported artifacts beyond the CRC, a whole-build comptime step budget, cyclic artifact references as a fuzz target"),
  ...
  ("Generated-source provenance and editability (M2.5 H19)", 0.45, "A `<file>.e.map.json` beside a generated file maps diagnostics back to the original span with the generated one related; a stale or malformed map is E-TOOL-0001 with no artifact (D264, D300); version 2 names the generator and hashes its input, so a changed input is stale too, and a mapping's `edit` says whether the generated span may be edited or the original is the target (D373); no fix is ever offered on an original span; the compiler runs no generator. a generated file whose hash is not the map's while the generator's input is unchanged is a hand edit, `E-TOOL-0002`, told from a stale map, pinned by the corpus (D418). Not yet: one input to many declarations, nested maps, provenance through specialisation and inlining, maps in a cache identity"),
