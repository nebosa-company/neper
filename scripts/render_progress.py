...
compiler = {
 "Front end and language": [
  ...
  ("Spec 11 debug check table and trap protocol", 0.45, "A check that fires follows the trap protocol (D194): the runtime's `neper_trap` writes `file:line:col: trap[kind]: <values>` to stderr and exits 134 on both platforms, and link/trap_bounds pins the record for an index and a slice past the end. Rows delivered: `bounds` (index and slice), the compiler's own `unreachable` points, and the `unreachable()` builtin with its literal as the values, which the checker counts as diverging (D195, link/trap_unreachable, check/unreachable_argument); `divide` traps by hardware. Not yet: `overflow`, `narrow`, `shift`, `enum`, `tag`, `null`, `align`, `@nocheck`, the release-mode elision, the symbolised backtrace and the test root's control handle"),
