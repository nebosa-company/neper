...
compiler = {
 "Front end and language": [
  ...
  ("Compiler responsiveness and correctness (M2.5 H10)", 0.6, "Phases separated so a check links and generates nothing (D314, D320); artifacts keyed by source, mode and interface hashes and published atomically (D205-D214, D343, D369); mutation fuzzing and a nesting bound (D341); deterministic failure codes for every exhausted limit (D344); images byte-equal across worker counts and perturbed schedules (D331); stage-2 == stage-3 and clean == incremental in both suites; one-shot query latency measured -- 25 ms for a small check, 269 ms for the whole compiler -- and no daemon adopted on it (D372), a batch of queries over one check instead (D409: twenty queries 447 ms, not 6.6 s); the crew's workers set themselves up on their own threads, the body phase of the compiler's cold release build 134 -> 69 ms at eight workers (D402). `test-impact-file --changed m1,m2` names the tests an edit reaches over the resolved call graph, pinned by the corpus over the test project (D423), and `test-file --only n1,n2` runs those alone (D424). Not yet: differential execution against an independent oracle, metamorphic tests, cancellation finer than `--deadline`'s checkpoints between phases (D399) and between a worker's modules (D422)"),
