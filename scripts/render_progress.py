...
compiler = {
 ...
 "Back end": [
  ...
  ("Incremental rebuild on the edge rule", 0.65, "`emit-em-all --incremental` (D205) compiles the program and then lets section 12's edge rule decide per module: the artifact on disk stays when its source hash and build mode are unchanged (`--release` builds release artifacts, D211) and every recorded edge still matches the declaration it names in the target's fresh artifact, and is replaced otherwise; `kept`/`rebuilt` is printed per module. link/incremental pins unchanged sources kept, a body edit behind a signature edge rebuilding only its module with the linked result byte-equal to a clean build, and a signature edit rebuilding the dependent, on both platforms. The artifact path is now usable on the compiler itself (D213): `emit-em-all` over its 31 modules took 289 s and takes 23 s, the incremental decision over them took 6 m 45 s and takes 5 s, and `link-em` of the result 6 s -- a CRC per read and a bit-loop `xor` under it were the cost, not the compiler. What is left is the saving: every module is still compiled and only the writes are skipped, because checking one module against its dependencies' Interfaces alone, without their sources, is not a path the checker has"),
