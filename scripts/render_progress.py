...
compiler = {
 "Front end and language": [
  ...
  ("Vec[T,N] / Mask[T,N] and SIMD lowering", 0.55, "`Vec[T, N]` and `Mask[T, N]` are builtins seeded into e.simd with section 4's closed table, width-aligned layout and `meta.kind/element_type/array_len` answers; every `simd.*` intrinsic but `shuffle` is library source lowered lane by lane over scalar code (D148); section 4's operator table -- IEEE `+ - * /` on float lanes, only `+% -% *%`, the bitwise ones and a scalar shift on integer lanes, `& | ^ ~` on masks -- is checked and lowered lane by lane, plain `+` on integer lanes and every comparison refused (D158). Not yet: the `Vec{}` literal and `v[i]`, the mask register-only rule, `shuffle`'s comptime array, and a vector register class with SSE/AVX selection"),
