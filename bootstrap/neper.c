...
#define OS_FN(source_name, native_name) fn = intrinsic_function(c, token, source_name, native_name)
    ...
    OS_FN("os.stdin", "neper_os_stdin"); intrinsic_returns(fn, 1, file, error);
    ...
    /* `os.sha256_blocks(state, bytes)` (D332): the whole blocks compressed with the SHA extensions, or none. */
    OS_FN("os.sha256_blocks", "neper_os_sha256_blocks"); intrinsic_param(fn, token, "state", intrinsic_slice(usize, 0)); intrinsic_param(fn, token, "bytes", const_bytes); intrinsic_returns(fn, 1, usize, error);
    /* `os.crc32c_bytes(crc, bytes)` (D332): the bytes folded with the CRC32 instruction, or none. */
    OS_FN("os.crc32c_bytes", "neper_os_crc32c_bytes"); intrinsic_param(fn, token, "crc", intrinsic_slice(usize, 0)); intrinsic_param(fn, token, "bytes", const_bytes); intrinsic_returns(fn, 1, usize, error);
...
static int assemble_and_link(Compiler *c, const char *asm_path, const char *obj_path,
    ...
    if (windows) {
        ...
        if (!f) return 0;
        fprintf(f, "@echo off\r\ncall %s -arch=x64 -host_arch=x64 >nul\r\n", qd);
        fprintf(f, "link /nologo /debug:full /include:np_nepersym /subsystem:console /entry:mainCRTStartup /out:%s %s %s kernel32.lib shell32.lib advapi32.lib >nul || exit /b 1\r\n", qe, qo, qr);
        fclose(f);
        if (command_status(command) != 0) return 0;
        remove(batch);
    } else {
