...
// AES uses the algebraic, constant-control-flow S-box in `e.crypto.aead` and has
// no secret-indexed lookup tables.
...
fn inverse_sbox(index: u8) -> u8 {
    let affine = aead.rotl8(index, 1u8) ^ aead.rotl8(index, 3u8) ^ aead.rotl8(index, 6u8) ^ 5u8
    ret aead.gf8_inverse(affine)
}

// a * b in GF(2^8) with the AES polynomial, by shift-and-add.
fn gf_mul(a: u8, b: u8) -> u8 {
    ret aead.gf8_mul(a, b)
}
