// Key derivation over `e.crypto.mac` and `e.crypto.hash`. HKDF by RFC 5869: extract is
// HMAC(salt, ikm) with an absent salt standing for a block of zeros, and expand is the
// chain T(i) = HMAC(prk, T(i-1) || info || i) for as many blocks as `dst` needs, at most
// 255 -- refused before a byte is written. The password hashes: PBKDF2 (RFC 8018) over
// HMAC-SHA-256 and -SHA-512, scrypt (RFC 7914) with Salsa20/8 BlockMix and ROMix over a
// caller-supplied scratch, bcrypt (the OpenBSD `$2b$` scheme: EksBlowfish with the
// expensive key schedule and 64 encryptions of "OrpheanBeholderScryDoubt"), and Argon2id
// (RFC 9106) over BLAKE2b with a caller-supplied memory of `m_kib` kibibytes. Nothing
// here allocates; nothing wipes the caller's buffers.

use e.crypto.hash as hash
use e.crypto.mac as mac

error TooLarge
error TooSmall
error Invalid

type Hash = enum u8 { Sha256, Sha512 }
type Blowfish = struct { p: [18]u32, s: [1024]u32 }

fn hkdf_sha256_extract(salt: []const u8, input_key: []const u8) -> [32]u8 {
    if salt.len == 0usize {
        let zeros: [32]u8 = zero
        ret mac.hmac_sha256(zeros[0..], input_key)
    }
    ret mac.hmac_sha256(salt, input_key)
}

fn hkdf_sha256_expand(dst: []u8, prk: [32]u8, info: []const u8) -> err {
    if dst.len > 255usize * 32usize { ret TooLarge }
    var previous: [32]u8 = zero
    var previous_len = 0usize
    var written = 0usize
    var counter = 1u8
    while written < dst.len {
        var s = mac.sha256_init(prk[0..])
        mac.sha256_update(&s, previous[..previous_len])
        mac.sha256_update(&s, info)
        var index: [1]u8 = zero
        index[0] = counter
        mac.sha256_update(&s, index[0..])
        previous = mac.sha256_done(&s)
        previous_len = 32usize
        var at = 0usize
        while at < 32usize && written < dst.len {
            dst[written] = previous[at]
            written += 1usize
            at += 1usize
        }
        counter += 1u8
    }
    ret ok
}

fn hkdf_sha512_extract(salt: []const u8, input_key: []const u8) -> [64]u8 {
    if salt.len == 0usize {
        let zeros: [64]u8 = zero
        ret mac.hmac_sha512(zeros[0..], input_key)
    }
    ret mac.hmac_sha512(salt, input_key)
}

fn hkdf_sha512_expand(dst: []u8, prk: [64]u8, info: []const u8) -> err {
    if dst.len > 255usize * 64usize { ret TooLarge }
    var previous: [64]u8 = zero
    var previous_len = 0usize
    var written = 0usize
    var counter = 1u8
    while written < dst.len {
        var s = mac.sha512_init(prk[0..])
        mac.sha512_update(&s, previous[..previous_len])
        mac.sha512_update(&s, info)
        var index: [1]u8 = zero
        index[0] = counter
        mac.sha512_update(&s, index[0..])
        previous = mac.sha512_done(&s)
        previous_len = 64usize
        var at = 0usize
        while at < 64usize && written < dst.len {
            dst[written] = previous[at]
            written += 1usize
            at += 1usize
        }
        counter += 1u8
    }
    ret ok
}

// --- PBKDF2. The HMAC state keyed with the password is built once and copied per
// iteration, so each U(i) costs the two compressions of its message alone.

fn store_be32(out: []u8, at: usize, value: u32) {
    out[at] = u8(value >> 24u32)
    out[at + 1usize] = u8((value >> 16u32) & 255u32)
    out[at + 2usize] = u8((value >> 8u32) & 255u32)
    out[at + 3usize] = u8(value & 255u32)
}

fn store_le32(out: []u8, at: usize, value: u32) {
    out[at] = u8(value & 255u32)
    out[at + 1usize] = u8((value >> 8u32) & 255u32)
    out[at + 2usize] = u8((value >> 16u32) & 255u32)
    out[at + 3usize] = u8(value >> 24u32)
}

fn load_le32(bytes: []const u8, at: usize) -> u32 {
    let low = u32(bytes[at]) | (u32(bytes[at + 1usize]) << 8u32)
    ret low | (u32(bytes[at + 2usize]) << 16u32) | (u32(bytes[at + 3usize]) << 24u32)
}

fn load_le64(bytes: []const u8, at: usize) -> u64 {
    ret u64(load_le32(bytes, at)) | (u64(load_le32(bytes, at + 4usize)) << 32u32)
}

fn store_le64(out: []u8, at: usize, value: u64) {
    store_le32(out, at, u32(value & 4294967295u64))
    store_le32(out, at + 4usize, u32(value >> 32u32))
}

fn pbkdf2_sha256(password: []const u8, salt: []const u8, iterations: u32, out: []u8) -> err {
    if iterations == 0u32 { ret Invalid }
    let base = mac.sha256_init(password)
    var block = 1u32
    var written = 0usize
    while written < out.len {
        var index: [4]u8 = zero
        store_be32(index[0..], 0usize, block)
        var s = base
        mac.sha256_update(&s, salt)
        mac.sha256_update(&s, index[0..])
        var u = mac.sha256_done(&s)
        var t = u
        var i = 1u32
        while i < iterations {
            var next_state = base
            mac.sha256_update(&next_state, u[0..])
            u = mac.sha256_done(&next_state)
            var k = 0usize
            while k < 32usize {
                t[k] = t[k] ^ u[k]
                k += 1usize
            }
            i += 1u32
        }
        var at = 0usize
        while at < 32usize && written < out.len {
            out[written] = t[at]
            written += 1usize
            at += 1usize
        }
        block += 1u32
    }
    ret ok
}

fn pbkdf2_sha512(password: []const u8, salt: []const u8, iterations: u32, out: []u8) -> err {
    if iterations == 0u32 { ret Invalid }
    let base = mac.sha512_init(password)
    var block = 1u32
    var written = 0usize
    while written < out.len {
        var index: [4]u8 = zero
        store_be32(index[0..], 0usize, block)
        var s = base
        mac.sha512_update(&s, salt)
        mac.sha512_update(&s, index[0..])
        var u = mac.sha512_done(&s)
        var t = u
        var i = 1u32
        while i < iterations {
            var next_state = base
            mac.sha512_update(&next_state, u[0..])
            u = mac.sha512_done(&next_state)
            var k = 0usize
            while k < 64usize {
                t[k] = t[k] ^ u[k]
                k += 1usize
            }
            i += 1u32
        }
        var at = 0usize
        while at < 64usize && written < out.len {
            out[written] = t[at]
            written += 1usize
            at += 1usize
        }
        block += 1u32
    }
    ret ok
}

fn pbkdf2(which: Hash, password: []const u8, salt: []const u8, iterations: u32, out: []u8) -> err {
    if which == .Sha256 { ret pbkdf2_sha256(password, salt, iterations, out) }
    ret pbkdf2_sha512(password, salt, iterations, out)
}

// --- scrypt. The scratch holds B (128 r p bytes), V (128 r N bytes) and the X and Y
// blocks of BlockMix (128 r each), in that order.

fn rotl32(x: u32, n: u32) -> u32 { ret (x << n) | (x >> (32u32 - n)) }

fn salsa_quarter(x: []u32, a: usize, b: usize, c: usize, d: usize) {
    x[b] = x[b] ^ rotl32(x[a] +% x[d], 7u32)
    x[c] = x[c] ^ rotl32(x[b] +% x[a], 9u32)
    x[d] = x[d] ^ rotl32(x[c] +% x[b], 13u32)
    x[a] = x[a] ^ rotl32(x[d] +% x[c], 18u32)
}

fn salsa20_8(block: []u8) {
    var x: [16]u32 = zero
    var i = 0usize
    while i < 16usize {
        x[i] = load_le32(block, i * 4usize)
        i += 1usize
    }
    var round = 0usize
    while round < 4usize {
        salsa_quarter(x[0..], 0usize, 4usize, 8usize, 12usize)
        salsa_quarter(x[0..], 5usize, 9usize, 13usize, 1usize)
        salsa_quarter(x[0..], 10usize, 14usize, 2usize, 6usize)
        salsa_quarter(x[0..], 15usize, 3usize, 7usize, 11usize)
        salsa_quarter(x[0..], 0usize, 1usize, 2usize, 3usize)
        salsa_quarter(x[0..], 5usize, 6usize, 7usize, 4usize)
        salsa_quarter(x[0..], 10usize, 11usize, 8usize, 9usize)
        salsa_quarter(x[0..], 15usize, 12usize, 13usize, 14usize)
        round += 1usize
    }
    i = 0usize
    while i < 16usize {
        store_le32(block, i * 4usize, load_le32(block, i * 4usize) +% x[i])
        i += 1usize
    }
}

// BlockMix over the 2r 64-byte blocks at `scratch[b..]`, using the 128r bytes at `y` as
// the intermediate Y; the result lands back at `b`.
fn scrypt_block_mix(scratch: []u8, b: usize, y: usize, r: usize) {
    var x: [64]u8 = zero
    var i = 0usize
    while i < 64usize {
        x[i] = scratch[b + (2usize * r - 1usize) * 64usize + i]
        i += 1usize
    }
    var block = 0usize
    while block < 2usize * r {
        i = 0usize
        while i < 64usize {
            x[i] = x[i] ^ scratch[b + block * 64usize + i]
            i += 1usize
        }
        salsa20_8(x[0..])
        i = 0usize
        while i < 64usize {
            scratch[y + block * 64usize + i] = x[i]
            i += 1usize
        }
        block += 1usize
    }
    block = 0usize
    while block < r {
        i = 0usize
        while i < 64usize {
            scratch[b + block * 64usize + i] = scratch[y + 2usize * block * 64usize + i]
            scratch[b + (r + block) * 64usize + i] = scratch[y + (2usize * block + 1usize) * 64usize + i]
            i += 1usize
        }
        block += 1usize
    }
}

fn scrypt_ro_mix(scratch: []u8, b: usize, v: usize, x: usize, y: usize, n: usize, r: usize) {
    let size = 128usize * r
    var i = 0usize
    while i < size {
        scratch[x + i] = scratch[b + i]
        i += 1usize
    }
    var step = 0usize
    while step < n {
        i = 0usize
        while i < size {
            scratch[v + step * size + i] = scratch[x + i]
            i += 1usize
        }
        scrypt_block_mix(scratch, x, y, r)
        step += 1usize
    }
    step = 0usize
    while step < n {
        let j = usize(load_le64(scratch, x + (2usize * r - 1usize) * 64usize) & u64(n - 1usize))
        i = 0usize
        while i < size {
            scratch[x + i] = scratch[x + i] ^ scratch[v + j * size + i]
            i += 1usize
        }
        scrypt_block_mix(scratch, x, y, r)
        step += 1usize
    }
    i = 0usize
    while i < size {
        scratch[b + i] = scratch[x + i]
        i += 1usize
    }
}

fn scrypt_scratch_required(n: usize, r: usize, p: usize) -> usize { ret 128usize * r * p + 128usize * r * n + 256usize * r }

// `n` is a power of two above 1; `scratch` holds at least `scrypt_scratch_required`.
fn scrypt(password: []const u8, salt: []const u8, n: usize, r: usize, p: usize, out: []u8, scratch: []u8) -> err {
    if n < 2usize || (n & (n - 1usize)) != 0usize || r == 0usize || p == 0usize { ret Invalid }
    if scratch.len < scrypt_scratch_required(n, r, p) { ret TooSmall }
    let b_len = 128usize * r * p
    let v = b_len
    let x = v + 128usize * r * n
    let y = x + 128usize * r
    if pbkdf2_sha256(password, salt, 1u32, scratch[..b_len]) != ok { ret Invalid }
    var lane = 0usize
    while lane < p {
        scrypt_ro_mix(scratch, lane * 128usize * r, v, x, y, n, r)
        lane += 1usize
    }
    ret pbkdf2_sha256(password, scratch[..b_len], 1u32, out)
}

// --- bcrypt. The Blowfish P-array and S-boxes are the first 4168 bytes of pi's
// fraction, big-endian words; the key and the salt are streamed cyclically in words.

fn blowfish_pi() -> str { ret "" }

fn blowfish_init() -> Blowfish {
    let digits = blowfish_pi()
    var bf: Blowfish = zero
    var i = 0usize
    while i < 18usize {
        bf.p[i] = (u32(digits[i * 4usize]) << 24u32) | (u32(digits[i * 4usize + 1usize]) << 16u32) | (u32(digits[i * 4usize + 2usize]) << 8u32) | u32(digits[i * 4usize + 3usize])
        i += 1usize
    }
    i = 0usize
    while i < 1024usize {
        let at = 72usize + i * 4usize
        bf.s[i] = (u32(digits[at]) << 24u32) | (u32(digits[at + 1usize]) << 16u32) | (u32(digits[at + 2usize]) << 8u32) | u32(digits[at + 3usize])
        i += 1usize
    }
    ret bf
}

fn blowfish_f(bf: *Blowfish, x: u32) -> u32 {
    let h = bf.s[usize(x >> 24u32)] +% bf.s[256usize + usize((x >> 16u32) & 255u32)]
    ret (h ^ bf.s[512usize + usize((x >> 8u32) & 255u32)]) +% bf.s[768usize + usize(x & 255u32)]
}

fn blowfish_encrypt(bf: *Blowfish, left: u32, right: u32) -> (u32, u32) {
    var l = left
    var r = right
    var i = 0usize
    while i < 16usize {
        l = l ^ bf.p[i]
        r = r ^ blowfish_f(bf, l)
        let swap = l
        l = r
        r = swap
        i += 1usize
    }
    ret (r ^ bf.p[17], l ^ bf.p[16])
}

// The next big-endian word of `data` read cyclically from `at`; an empty `data` is zeros.
fn stream_word(data: []const u8, at: usize) -> (u32, usize) {
    if data.len == 0usize { ret (0u32, 0usize) }
    var w = 0u32
    var j = at
    var k = 0usize
    while k < 4usize {
        w = (w << 8u32) | u32(data[j])
        j += 1usize
        if j == data.len { j = 0usize }
        k += 1usize
    }
    ret (w, j)
}

fn blowfish_expand(bf: *Blowfish, salt: []const u8, key: []const u8) {
    var j = 0usize
    var i = 0usize
    while i < 18usize {
        let (w, next_j) = stream_word(key, j)
        bf.p[i] = bf.p[i] ^ w
        j = next_j
        i += 1usize
    }
    var l = 0u32
    var r = 0u32
    j = 0usize
    i = 0usize
    while i < 18usize {
        let (sl, j1) = stream_word(salt, j)
        let (sr, j2) = stream_word(salt, j1)
        j = j2
        let (el, er) = blowfish_encrypt(bf, l ^ sl, r ^ sr)
        l = el
        r = er
        bf.p[i] = l
        bf.p[i + 1usize] = r
        i += 2usize
    }
    i = 0usize
    while i < 1024usize {
        let (sl, j1) = stream_word(salt, j)
        let (sr, j2) = stream_word(salt, j1)
        j = j2
        let (el, er) = blowfish_encrypt(bf, l ^ sl, r ^ sr)
        l = el
        r = er
        bf.s[i] = l
        bf.s[i + 1usize] = r
        i += 2usize
    }
}

// The 23 raw bytes of bcrypt for `cost` in 4..31; the password is truncated to 72 bytes
// counting the terminating zero the scheme appends.
fn bcrypt(password: []const u8, salt: [16]u8, cost: u32) -> ([23]u8, err) {
    var out: [23]u8 = zero
    if cost < 4u32 || cost > 31u32 { ret (out, Invalid) }
    var key: [72]u8 = zero
    var key_len = password.len + 1usize
    if key_len > 72usize { key_len = 72usize }
    var i = 0usize
    while i < key_len && i < password.len {
        key[i] = password[i]
        i += 1usize
    }
    var bf = blowfish_init()
    let none: [0]u8 = zero
    blowfish_expand(&bf, salt[0..], key[..key_len])
    var round = 0u64
    let rounds = 1u64 << cost
    while round < rounds {
        blowfish_expand(&bf, none[0..], key[..key_len])
        blowfish_expand(&bf, none[0..], salt[0..])
        round += 1u64
    }
    let magic = "OrpheanBeholderScryDoubt"
    var words: [6]u32 = zero
    i = 0usize
    while i < 6usize {
        words[i] = (u32(magic[i * 4usize]) << 24u32) | (u32(magic[i * 4usize + 1usize]) << 16u32) | (u32(magic[i * 4usize + 2usize]) << 8u32) | u32(magic[i * 4usize + 3usize])
        i += 1usize
    }
    round = 0u64
    while round < 64u64 {
        i = 0usize
        while i < 6usize {
            let (l, r) = blowfish_encrypt(&bf, words[i], words[i + 1usize])
            words[i] = l
            words[i + 1usize] = r
            i += 2usize
        }
        round += 1u64
    }
    var text: [24]u8 = zero
    i = 0usize
    while i < 6usize {
        store_be32(text[0..], i * 4usize, words[i])
        i += 1usize
    }
    i = 0usize
    while i < 23usize {
        out[i] = text[i]
        i += 1usize
    }
    ret (out, ok)
}

fn bcrypt_alphabet(value: u8) -> u8 {
    let alphabet = "./ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"
    ret alphabet[usize(value & 63u8)]
}

// 255 for a byte outside the alphabet.
fn bcrypt_digit(c: u8) -> u8 {
    if c == 46u8 { ret 0u8 }
    if c == 47u8 { ret 1u8 }
    if c >= 65u8 && c <= 90u8 { ret c - 65u8 + 2u8 }
    if c >= 97u8 && c <= 122u8 { ret c - 97u8 + 28u8 }
    if c >= 48u8 && c <= 57u8 { ret c - 48u8 + 54u8 }
    ret 255u8
}

// bcrypt's base64 (no padding) of `data` into `out`, which holds ceil(4 len / 3) bytes.
fn bcrypt_encode(out: []u8, data: []const u8) {
    var i = 0usize
    var o = 0usize
    while i < data.len {
        let c1 = data[i]
        i += 1usize
        out[o] = bcrypt_alphabet(c1 >> 2u32)
        o += 1usize
        var rest = (c1 & 3u8) << 4u32
        if i >= data.len {
            out[o] = bcrypt_alphabet(rest)
            ret
        }
        let c2 = data[i]
        i += 1usize
        out[o] = bcrypt_alphabet(rest | (c2 >> 4u32))
        o += 1usize
        rest = (c2 & 15u8) << 2u32
        if i >= data.len {
            out[o] = bcrypt_alphabet(rest)
            ret
        }
        let c3 = data[i]
        i += 1usize
        out[o] = bcrypt_alphabet(rest | (c3 >> 6u32))
        out[o + 1usize] = bcrypt_alphabet(c3 & 63u8)
        o += 2usize
    }
}

fn bcrypt_decode_salt(text: []const u8) -> ([16]u8, bool) {
    var out: [16]u8 = zero
    var digits: [22]u8 = zero
    var i = 0usize
    while i < 22usize {
        digits[i] = bcrypt_digit(text[i])
        if digits[i] == 255u8 { ret (out, false) }
        i += 1usize
    }
    var o = 0usize
    i = 0usize
    while o < 16usize {
        out[o] = (digits[i] << 2u32) | (digits[i + 1usize] >> 4u32)
        o += 1usize
        if o < 16usize {
            out[o] = (digits[i + 1usize] << 4u32) | (digits[i + 2usize] >> 2u32)
            o += 1usize
        }
        if o < 16usize {
            out[o] = (digits[i + 2usize] << 6u32) | digits[i + 3usize]
            o += 1usize
        }
        i += 4usize
    }
    ret (out, true)
}

// The 60-byte `$2b$NN$` text: the cost as two digits, 22 characters of salt, 31 of hash.
fn bcrypt_hash_text(password: []const u8, salt: [16]u8, cost: u32) -> ([60]u8, err) {
    var out: [60]u8 = zero
    let (raw, bcrypt_error) = bcrypt(password, salt, cost)
    if bcrypt_error != ok { ret (out, bcrypt_error) }
    out[0] = 36u8
    out[1] = 50u8
    out[2] = 98u8
    out[3] = 36u8
    out[4] = 48u8 + u8(cost / 10u32)
    out[5] = 48u8 + u8(cost % 10u32)
    out[6] = 36u8
    bcrypt_encode(out[7..29], salt[0..])
    bcrypt_encode(out[29..60], raw[0..])
    ret (out, ok)
}

// True when `hash_text` (a `$2a$`, `$2b$` or `$2y$` string) is `password`'s; the
// comparison of the hash characters takes the same time whatever they are.
fn bcrypt_verify(password: []const u8, hash_text: []const u8) -> bool {
    if hash_text.len != 60usize { ret false }
    if hash_text[0] != 36u8 || hash_text[1] != 50u8 || hash_text[3] != 36u8 || hash_text[6] != 36u8 { ret false }
    if hash_text[2] != 97u8 && hash_text[2] != 98u8 && hash_text[2] != 121u8 { ret false }
    if hash_text[4] < 48u8 || hash_text[4] > 57u8 || hash_text[5] < 48u8 || hash_text[5] > 57u8 { ret false }
    let cost = u32(hash_text[4] - 48u8) * 10u32 + u32(hash_text[5] - 48u8)
    let (salt, salt_ok) = bcrypt_decode_salt(hash_text[7..29])
    if !salt_ok { ret false }
    let (raw, bcrypt_error) = bcrypt(password, salt, cost)
    if bcrypt_error != ok { ret false }
    var encoded: [31]u8 = zero
    bcrypt_encode(encoded[0..], raw[0..])
    ret hash.equal_constant_time(encoded[0..], hash_text[29..60])
}

// --- Argon2id. Blocks are 1024 bytes of `memory`, read and written as little-endian
// words; the compression G works on two 128-word arrays so the address blocks of the
// data-independent slices never touch `memory`.

fn argon2_memory_required(m_kib: u32) -> usize { ret usize(m_kib) * 1024usize }

fn argon2_gb(v: []u64, a: usize, b: usize, c: usize, d: usize) {
    v[a] = v[a] +% v[b] +% 2u64 *% (v[a] & 4294967295u64) *% (v[b] & 4294967295u64)
    v[d] = hash_rotr64(v[d] ^ v[a], 32u32)
    v[c] = v[c] +% v[d] +% 2u64 *% (v[c] & 4294967295u64) *% (v[d] & 4294967295u64)
    v[b] = hash_rotr64(v[b] ^ v[c], 24u32)
    v[a] = v[a] +% v[b] +% 2u64 *% (v[a] & 4294967295u64) *% (v[b] & 4294967295u64)
    v[d] = hash_rotr64(v[d] ^ v[a], 16u32)
    v[c] = v[c] +% v[d] +% 2u64 *% (v[c] & 4294967295u64) *% (v[d] & 4294967295u64)
    v[b] = hash_rotr64(v[b] ^ v[c], 63u32)
}

fn hash_rotr64(x: u64, n: u32) -> u64 { ret (x >> n) | (x << (64u32 - n)) }

// The permutation P over the 16 words z[first + (k / 2) pair_step + (k % 2)].
fn argon2_permute(z: []u64, first: usize, pair_step: usize) {
    var v: [16]u64 = zero
    var k = 0usize
    while k < 16usize {
        v[k] = z[first + (k / 2usize) * pair_step + (k % 2usize)]
        k += 1usize
    }
    argon2_gb(v[0..], 0usize, 4usize, 8usize, 12usize)
    argon2_gb(v[0..], 1usize, 5usize, 9usize, 13usize)
    argon2_gb(v[0..], 2usize, 6usize, 10usize, 14usize)
    argon2_gb(v[0..], 3usize, 7usize, 11usize, 15usize)
    argon2_gb(v[0..], 0usize, 5usize, 10usize, 15usize)
    argon2_gb(v[0..], 1usize, 6usize, 11usize, 12usize)
    argon2_gb(v[0..], 2usize, 7usize, 8usize, 13usize)
    argon2_gb(v[0..], 3usize, 4usize, 9usize, 14usize)
    k = 0usize
    while k < 16usize {
        z[first + (k / 2usize) * pair_step + (k % 2usize)] = v[k]
        k += 1usize
    }
}

// G's core: `z` starts as R = X ^ Y and ends as P(R) ^ R.
fn argon2_g_core(z: []u64) {
    var r: [128]u64 = zero
    var i = 0usize
    while i < 128usize {
        r[i] = z[i]
        i += 1usize
    }
    i = 0usize
    while i < 8usize {
        argon2_permute(z, 16usize * i, 2usize)
        i += 1usize
    }
    i = 0usize
    while i < 8usize {
        argon2_permute(z, 2usize * i, 16usize)
        i += 1usize
    }
    i = 0usize
    while i < 128usize {
        z[i] = z[i] ^ r[i]
        i += 1usize
    }
}

// memory[cur] = G(memory[prev], memory[ref]), XORed into the old block on later passes.
fn argon2_fill(memory: []u8, prev: usize, ref: usize, cur: usize, with_xor: bool) {
    var z: [128]u64 = zero
    var i = 0usize
    while i < 128usize {
        z[i] = load_le64(memory, prev + i * 8usize) ^ load_le64(memory, ref + i * 8usize)
        i += 1usize
    }
    argon2_g_core(z[0..])
    i = 0usize
    while i < 128usize {
        var word = z[i]
        if with_xor { word = word ^ load_le64(memory, cur + i * 8usize) }
        store_le64(memory, cur + i * 8usize, word)
        i += 1usize
    }
}

// addresses = G(0, G(0, input)) with input's counter first advanced.
fn argon2_next_addresses(input: []u64, addresses: []u64) {
    input[6] += 1u64
    var i = 0usize
    while i < 128usize {
        addresses[i] = input[i]
        i += 1usize
    }
    argon2_g_core(addresses)
    argon2_g_core(addresses)
}

// H'(T, X) for T = out.len: BLAKE2b-T of LE32(T) || X when T is at most 64, else the
// 32-byte prefixes of a BLAKE2b-64 chain with a final digest of the leftover length.
fn argon2_h_prime(out: []u8, input: []const u8) {
    let none: [0]u8 = zero
    var length: [4]u8 = zero
    store_le32(length[0..], 0usize, u32(out.len))
    if out.len <= 64usize {
        var s = hash.blake2b_init(out.len, none[0..])
        hash.blake2b_update(&s, length[0..])
        hash.blake2b_update(&s, input)
        let digest = hash.blake2b_done(&s)
        var i = 0usize
        while i < out.len {
            out[i] = digest[i]
            i += 1usize
        }
        ret
    }
    let r = (out.len + 31usize) / 32usize - 2usize
    var s = hash.blake2b_init(64usize, none[0..])
    hash.blake2b_update(&s, length[0..])
    hash.blake2b_update(&s, input)
    var v = hash.blake2b_done(&s)
    var written = 0usize
    var step = 0usize
    while step < r {
        var i = 0usize
        while i < 32usize {
            out[written + i] = v[i]
            i += 1usize
        }
        written += 32usize
        if step + 1usize < r { v = hash.blake2b(v[0..]) }
        step += 1usize
    }
    var last = hash.blake2b_init(out.len - written, none[0..])
    hash.blake2b_update(&last, v[0..])
    let tail = hash.blake2b_done(&last)
    var i = 0usize
    while written < out.len {
        out[written] = tail[i]
        written += 1usize
        i += 1usize
    }
}

fn argon2_update_le32(s: *hash.Blake2b, value: u32) {
    var word: [4]u8 = zero
    store_le32(word[0..], 0usize, value)
    hash.blake2b_update(s, word[0..])
}

fn argon2_update_sized(s: *hash.Blake2b, data: []const u8) {
    argon2_update_le32(s, u32(data.len))
    hash.blake2b_update(s, data)
}

fn argon2id(password: []const u8, salt: []const u8, t: u32, m_kib: u32, p: u32, out: []u8, memory: []u8) -> err {
    let none: [0]u8 = zero
    ret argon2id_keyed(password, salt, none[0..], none[0..], t, m_kib, p, out, memory)
}

// ponytail: Argon2id only (type 2); Argon2i/Argon2d differ solely in the `independent`
// test and the type word, add a variant parameter when a caller needs them.
// Argon2id with the optional secret K and associated data X of RFC 9106; `memory` holds
// at least `argon2_memory_required(m_kib)` bytes, `t` passes, `p` lanes, `m_kib` at least
// 8 p, a tag of at least 4 bytes.
fn argon2id_keyed(password: []const u8, salt: []const u8, secret: []const u8, ad: []const u8, t: u32, m_kib: u32, p: u32, out: []u8, memory: []u8) -> err {
    if t == 0u32 || p == 0u32 || m_kib < 8u32 * p || out.len < 4usize || salt.len < 8usize { ret Invalid }
    if memory.len < argon2_memory_required(m_kib) { ret TooSmall }
    let none: [0]u8 = zero
    var h = hash.blake2b_init(64usize, none[0..])
    argon2_update_le32(&h, p)
    argon2_update_le32(&h, u32(out.len))
    argon2_update_le32(&h, m_kib)
    argon2_update_le32(&h, t)
    argon2_update_le32(&h, 19u32)
    argon2_update_le32(&h, 2u32)
    argon2_update_sized(&h, password)
    argon2_update_sized(&h, salt)
    argon2_update_sized(&h, secret)
    argon2_update_sized(&h, ad)
    let h0 = hash.blake2b_done(&h)
    let lanes = usize(p)
    let blocks = 4usize * lanes * (usize(m_kib) / (4usize * lanes))
    let q = blocks / lanes
    let segment = q / 4usize
    // The first two blocks of every lane come from H0.
    var seed: [72]u8 = zero
    var i = 0usize
    while i < 64usize {
        seed[i] = h0[i]
        i += 1usize
    }
    var lane = 0usize
    while lane < lanes {
        store_le32(seed[0..], 68usize, u32(lane))
        store_le32(seed[0..], 64usize, 0u32)
        argon2_h_prime(memory[lane * q * 1024usize..lane * q * 1024usize + 1024usize], seed[0..])
        store_le32(seed[0..], 64usize, 1u32)
        argon2_h_prime(memory[(lane * q + 1usize) * 1024usize..(lane * q + 2usize) * 1024usize], seed[0..])
        lane += 1usize
    }
    var pass = 0usize
    while pass < usize(t) {
        var slice = 0usize
        while slice < 4usize {
            lane = 0usize
            while lane < lanes {
                let independent = pass == 0usize && slice < 2usize
                var input: [128]u64 = zero
                var addresses: [128]u64 = zero
                input[0] = u64(pass)
                input[1] = u64(lane)
                input[2] = u64(slice)
                input[3] = u64(blocks)
                input[4] = u64(t)
                input[5] = 2u64
                var start = 0usize
                if pass == 0usize && slice == 0usize {
                    start = 2usize
                    if independent { argon2_next_addresses(input[0..], addresses[0..]) }
                }
                var index = start
                while index < segment {
                    let column = slice * segment + index
                    var prev_column = q - 1usize
                    if column > 0usize { prev_column = column - 1usize }
                    let prev = (lane * q + prev_column) * 1024usize
                    var random = 0u64
                    if independent {
                        if index % 128usize == 0usize { argon2_next_addresses(input[0..], addresses[0..]) }
                        random = addresses[index % 128usize]
                    } else {
                        random = load_le64(memory, prev)
                    }
                    var ref_lane = usize((random >> 32u32) % u64(lanes))
                    if pass == 0usize && slice == 0usize { ref_lane = lane }
                    let same_lane = ref_lane == lane
                    var area = 0usize
                    if pass == 0usize {
                        if slice == 0usize {
                            area = index - 1usize
                        } else {
                            area = slice * segment
                            if same_lane { area = area + index - 1usize } else { if index == 0usize { area -= 1usize } }
                        }
                    } else {
                        area = q - segment
                        if same_lane { area = area + index - 1usize } else { if index == 0usize { area -= 1usize } }
                    }
                    let j1 = random & 4294967295u64
                    let x = (j1 * j1) >> 32u32
                    let y = (u64(area) * x) >> 32u32
                    let relative = area - 1usize - usize(y)
                    var start_position = 0usize
                    if pass != 0usize && slice != 3usize { start_position = (slice + 1usize) * segment }
                    let ref_column = (start_position + relative) % q
                    let ref = (ref_lane * q + ref_column) * 1024usize
                    let cur = (lane * q + column) * 1024usize
                    argon2_fill(memory, prev, ref, cur, pass != 0usize)
                    index += 1usize
                }
                lane += 1usize
            }
            slice += 1usize
        }
        pass += 1usize
    }
    var last_blocks: [1024]u8 = zero
    lane = 0usize
    while lane < lanes {
        let last = (lane * q + q - 1usize) * 1024usize
        i = 0usize
        while i < 1024usize {
            last_blocks[i] = last_blocks[i] ^ memory[last + i]
            i += 1usize
        }
        lane += 1usize
    }
    argon2_h_prime(out, last_blocks[0..])
    ret ok
}
