...
$uiInputsV2Path = Join-Path $testBuild 'ui-inputs-v2-selfhost.exe'
$uiInputsV2Written = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\ui_inputs_v2\src\main.e') $repo 'x64' 'windows' $uiInputsV2Path
...
$uiSelectionV2Path = Join-Path $testBuild 'ui-selection-v2-selfhost.exe'
$uiSelectionV2Written = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\ui_selection_v2\src\main.e') $repo 'x64' 'windows' $uiSelectionV2Path
...
$uiSelection3V2Path = Join-Path $testBuild 'ui-selection3-v2-selfhost.exe'
$uiSelection3V2Written = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\ui_selection3_v2\src\main.e') $repo 'x64' 'windows' $uiSelection3V2Path
...
$uiPickers2V2Path = Join-Path $testBuild 'ui-pickers2-v2-selfhost.exe'
$uiPickers2V2Written = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\ui_pickers2_v2\src\main.e') $repo 'x64' 'windows' $uiPickers2V2Path
if ($LASTEXITCODE -ne 0 -or $uiPickers2V2Written -ne 'executable written') { throw 'ui_pickers2_v2 emission failed' }
$uiPickers2V2Output = & $uiPickers2V2Path
if ($LASTEXITCODE -ne 0 -or $uiPickers2V2Output -ne 'ui pickers2 v2 ok') { throw "ui_pickers2_v2 answered wrongly: exit $LASTEXITCODE" }
# v2 colour field and font panel (D961)
$uiPickers3V2Path = Join-Path $testBuild 'ui-pickers3-v2-selfhost.exe'
$uiPickers3V2Written = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\ui_pickers3_v2\src\main.e') $repo 'x64' 'windows' $uiPickers3V2Path
if ($LASTEXITCODE -ne 0 -or $uiPickers3V2Written -ne 'executable written') { throw 'ui_pickers3_v2 emission failed' }
$uiPickers3V2Output = & $uiPickers3V2Path
if ($LASTEXITCODE -ne 0 -or $uiPickers3V2Output -ne 'ui pickers3 v2 ok') { throw "ui_pickers3_v2 answered wrongly: exit $LASTEXITCODE" }
# v2 content controls: icons, avatars, images, canvases, headings (D962)
$uiContentV2Path = Join-Path $testBuild 'ui-content-v2-selfhost.exe'
$uiContentV2Written = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\ui_content_v2\src\main.e') $repo 'x64' 'windows' $uiContentV2Path
...
$uiContent3V2Path = Join-Path $testBuild 'ui-content3-v2-selfhost.exe'
$uiContent3V2Written = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\ui_content3_v2\src\main.e') $repo 'x64' 'windows' $uiContent3V2Path
if ($LASTEXITCODE -ne 0 -or $uiContent3V2Written -ne 'executable written') { throw 'ui_content3_v2 emission failed' }
$uiContent3V2Output = & $uiContent3V2Path
if ($LASTEXITCODE -ne 0 -or $uiContent3V2Output -ne 'ui content3 v2 ok') { throw "ui_content3_v2 answered wrongly: exit $LASTEXITCODE" }
# v2 dividers, cards, group boxes, disclosure, expander and accordion (D965, P5-08)
$uiContainersV2Path = Join-Path $testBuild 'ui-containers-v2-selfhost.exe'
$uiContainersV2Written = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\ui_containers_v2\src\main.e') $repo 'x64' 'windows' $uiContainersV2Path
if ($LASTEXITCODE -ne 0 -or $uiContainersV2Written -ne 'executable written') { throw 'ui_containers_v2 emission failed' }
$uiContainersV2Output = & $uiContainersV2Path
if ($LASTEXITCODE -ne 0 -or $uiContainersV2Output -ne 'ui containers v2 ok') { throw "ui_containers_v2 answered wrongly: exit $LASTEXITCODE" }
# v2 resizable pane, split view, dock panel, dock layout and workspace (D966, P5-08)
$uiContainers2V2Path = Join-Path $testBuild 'ui-containers2-v2-selfhost.exe'
$uiContainers2V2Written = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\ui_containers2_v2\src\main.e') $repo 'x64' 'windows' $uiContainers2V2Path
...
$uiContainers4V2Path = Join-Path $testBuild 'ui-containers4-v2-selfhost.exe'
$uiContainers4V2Written = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\ui_containers4_v2\src\main.e') $repo 'x64' 'windows' $uiContainers4V2Path
if ($LASTEXITCODE -ne 0 -or $uiContainers4V2Written -ne 'executable written') { throw 'ui_containers4_v2 emission failed' }
$uiContainers4V2Output = & $uiContainers4V2Path
if ($LASTEXITCODE -ne 0 -or $uiContainers4V2Output -ne 'ui containers4 v2 ok') { throw "ui_containers4_v2 answered wrongly: exit $LASTEXITCODE" }
# v2 document workspace over editor groups (D969, P5-08)
$uiContainers5V2Path = Join-Path $testBuild 'ui-containers5-v2-selfhost.exe'
$uiContainers5V2Written = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\ui_containers5_v2\src\main.e') $repo 'x64' 'windows' $uiContainers5V2Path
...
$uiFormPath = Join-Path $testBuild 'ui-form-selfhost.exe'
$uiFormWritten = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\ui_form\src\main.e') $repo 'x64' 'windows' $uiFormPath
...
# shortcut recorder.
$uiNumericPath = Join-Path $testBuild 'ui-numeric-selfhost.exe'
$uiNumericWritten = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\ui_numeric\src\main.e') $repo 'x64' 'windows' $uiNumericPath
if ($LASTEXITCODE -ne 0 -or $uiNumericWritten -ne 'executable written') { throw 'ui_numeric emission failed' }
$uiNumericOutput = & $uiNumericPath
if ($LASTEXITCODE -ne 0 -or $uiNumericOutput -ne 'ui numeric ok') { throw "the numeric controls answered wrongly: exit $LASTEXITCODE" }
...
$uiEntryPath = Join-Path $testBuild 'ui-entry-selfhost.exe'
$uiEntryWritten = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\ui_entry\src\main.e') $repo 'x64' 'windows' $uiEntryPath
...
$uiPickersPath = Join-Path $testBuild 'ui-pickers-selfhost.exe'
$uiPickersWritten = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\ui_pickers\src\main.e') $repo 'x64' 'windows' $uiPickersPath
...
$uiTabularPath = Join-Path $testBuild 'ui-tabular-selfhost.exe'
$uiTabularWritten = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\ui_tabular\src\main.e') $repo 'x64' 'windows' $uiTabularPath
...
$arithmeticUncheckedPath = Join-Path $testBuild 'trap-arithmetic-unchecked-selfhost.exe'
$arithmeticUncheckedWritten = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\trap_arithmetic\src\main.e') $repo 'x64' 'windows' $arithmeticUncheckedPath --unchecked
if ($LASTEXITCODE -ne 0 -or $arithmeticUncheckedWritten -ne 'executable written') { throw 'the debug --unchecked arithmetic build failed' }
& $arithmeticUncheckedPath shift 2>&1 | Out-Null
if ($LASTEXITCODE -ne 0) { throw "a debug --unchecked build kept the shift check: exit $LASTEXITCODE" }
$arithmeticUncheckedOutput = & $arithmeticUncheckedPath zero 2>&1
if ($LASTEXITCODE -ne 134 -or ($arithmeticUncheckedOutput -join "`n") -notmatch 'main\.e:14:17: trap\[divide\]: 7 / 0 divides by zero') { throw "a debug --unchecked build dropped the always-on divide check: exit $LASTEXITCODE" }
...
foreach ($layoutMode in @(@(), @('--release'))) {
    $layoutPath = Join-Path $testBuild 'layout-packed-selfhost.exe'
    $layoutWritten = & $compiler emit-executable (Join-Path $PSScriptRoot 'fixtures\link\layout_packed\src\main.e') $repo 'x64' 'windows' $layoutPath @layoutMode
    if ($LASTEXITCODE -ne 0 -or $layoutWritten -ne 'executable written') { throw 'packed layout fixture executable emission failed' }
    & $layoutPath 2>&1 | Out-Null
    if ($LASTEXITCODE -ne 0) { throw "the packed layout fixture exited $LASTEXITCODE $layoutMode" }
}
