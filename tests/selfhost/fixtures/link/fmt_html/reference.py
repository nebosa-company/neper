# Generates the named character reference table in lib/e/fmt/html.e from Python's
# html.entities (the HTML5 list) and writes src/main.e. Run from this directory.
import html.entities

entries = sorted(html.entities.html5.items())
table = b''
for name, value in entries:
    n = name.encode()
    v = value.encode()
    table += bytes([len(n)]) + n + bytes([len(v)]) + v
...
path = '../../../../../lib/e/fmt/html.e'
source = open(path, encoding='utf-8').read()
start = source.index('fn entity_table() -> str { ret ')
source = source[:start] + 'fn entity_table() -> str { ret ' + literal(table) + ' }\n'
open(path, 'w', encoding='utf-8', newline='\n').write(source)

fixture = r'''// `e.fmt.html`: a document with a DOCTYPE, a comment, head elements, attributes in
