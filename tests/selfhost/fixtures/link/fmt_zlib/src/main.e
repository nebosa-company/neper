// `e.fmt.zlib`: Python's zlib.compress of a 2280-byte text read back through the
// reader in 100-byte pulls, the text written through the writer at Balanced and read
// back, and the refusals: a flipped trailer byte (Checksum), a bad header (Invalid),
// an output limit below the size, storage too small. Every check has its own exit code.
use e.os
use e.mem
use e.io
use e.fmt.zlib as zlib
...
// Reads everything through a zlib reader over `framed` in 100-byte pulls.
fn inflate(storage: []u8, framed: []const u8, out: []u8, limit: u64) -> (usize, err) {
    var source_state = io.SliceReader { data: framed, off: 0usize }
    let (r0, reader_error) = zlib.reader(storage, io.slice_reader(&source_state), limit)
    ...
    while true {
        ...
        let (count, read_error) = zlib.read(&r, out[total..end])
...
fn main(a: *mem.Arena, args: []str) -> err {
    ...
    let (w0, w_error) = zlib.writer(storage, io.slice_writer(&sink_state), .Balanced)
    if w_error != ok { os.exit(7) }
    var w = w0
    let (c1, e2) = zlib.write(&w, text[..1000])
    if e2 != ok || c1 != 1000usize { os.exit(8) }
    let (c2, e3) = zlib.write(&w, text[1000..])
    if e3 != ok || c2 != 1280usize { os.exit(9) }
    if zlib.finish(&w) != ok { os.exit(10) }
    if packed[0] != 120u8 || packed[1] != 156u8 || sink_state.off >= text.len { os.exit(11) }
    let (n2, e4) = inflate(storage, packed[..sink_state.off], out, 1000000u64)
    if e4 != ok || !same(out[..n2], text) { os.exit(12) }
    // Refusals.
    var flipped = framed
    flipped[69] = flipped[69] ^ 1u8
    let (n3, e5) = inflate(storage, flipped[0..], out, 1000000u64)
    if e5 != zlib.Checksum { os.exit(13) }
    var bad_header = framed
    bad_header[1] = 217u8
    let (n4, e6) = inflate(storage, bad_header[0..], out, 1000000u64)
    if e6 != zlib.Invalid { os.exit(14) }
    let (n5, e7) = inflate(storage, framed[0..], out, 2000u64)
    if e7 == ok || e7 == zlib.Checksum || e7 == zlib.Invalid { os.exit(15) }
    let (n6, e8) = inflate(storage[..100], framed[0..], out, 1000000u64)
    if e8 != io.TooSmall { os.exit(16) }
    let (n7, e9) = inflate(storage, framed[0..60], out, 1000000u64)
    if e9 != zlib.Invalid { os.exit(17) }
    ret ok
}
