...
fn main(a: *mem.Arena, args: []str) -> err {
    ...
    children[1usize] = widget.edit(4u64, widget.Edit { buffer: buffer, len: 5usize, style: text_style, color: paint.rgba(1.0, 1.0, 1.0, 1.0), selection: paint.rgba(0.0, 0.0, 1.0, 1.0), change: widget.Change[str] { ctx: ctx, invoke: on_change }, submit: zero, enabled: true, read_only: false, multiline: false }, sized(40.0, 16.0))
    children[2usize] = widget.text(5u64, widget.Text { value: "Theme", style: text_style, color: paint.rgba(1.0, 1.0, 1.0, 1.0) }, style.defaults())
