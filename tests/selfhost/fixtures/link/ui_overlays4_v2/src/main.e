...
fn build(a: *mem.Arena, t: *const control.Theme, ctx: *void, dismiss: *const widget.Submit, buffer: []u8, which: Which, active: usize) -> (widget.Node, err) {
    ...
    if which == .PaletteGrouped {
        ...
        grouped[0usize] = navigation.PaletteCommand { name: names[0usize], group: "Recent", category: "Project" }
        grouped[1usize] = navigation.PaletteCommand { name: names[1usize], group: "Recent", category: "Project" }
        grouped[2usize] = navigation.PaletteCommand { name: names[2usize], group: "Commands", category: "Release" }
...
fn main(a: *mem.Arena, args: []str) -> err {
    ...
    if !has_recent_group || !has_commands_group || !has_categorized_option || testing.by_text(&harness, "Project:").count != 2usize || testing.by_text(&harness, "Release:").count != 1usize { os.exit(42i32) }
